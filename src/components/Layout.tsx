        ...
        <nav className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/" className="flex items-center space-x-2 group">
              <img 
                src={foursightLogo} 
                alt="FourSight Media & Tech Logo" 
                className="h-12 w-auto smooth-transition group-hover:scale-105"
              />
            </Link>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navigation.map((item) => (
                ...
                >
                  {item.name}
                  <span className={`absolute -bottom-1 left-0 h-0.5 bg-primary smooth-transition ${
              ...
              <Button
                ...
                size="icon"
                onClick={toggleTheme}
                className="smooth-transition hover:shadow-glow"
              >
              ...
              <div className="flex flex-col space-y-4">
                {navigation.map((item) => (
                  <Link
                    ...
                    className={`font-medium smooth-transition px-2 py-1 rounded-md ${
                      isActive(item.href)
      ...
      <footer className="bg-neutral text-neutral-foreground py-12">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div className="space-y-2">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    className="block text-neutral-foreground/80 hover:text-accent smooth-transition"
                  >
                    {item.name}
                  </Link>
                ))}
            ...
            <div>
              ...
              <div className="space-y-2 text-neutral-foreground/80">
                <p>Email: hello@foursightmedia.tech</p>
          ...
          <div className="border-t border-neutral-foreground/20 mt-8 pt-8 text-center text-neutral-foreground/60">
            <p>&copy; 2024 FourSight Media & Tech. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
